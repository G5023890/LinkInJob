...
APP_DISPLAY_NAME="${APP_DISPLAY_NAME:-Project Cleaner}"
EXECUTABLE_NAME="${EXECUTABLE_NAME:-ProjectCleanerApp}"
BUNDLE_ID="${BUNDLE_ID:-com.grigorym.projectcleaner}"
...
SCRIPT_SOURCE="$PROJECT_DIR/scripts/safe_clean.sh"
if [[ ! -x "$SCRIPT_SOURCE" ]]; then
  echo "Cleaner script not found or not executable: $SCRIPT_SOURCE" >&2
  exit 1
fi
mkdir -p "$APP_STAGE/Contents/Resources/scripts"
cp "$SCRIPT_SOURCE" "$APP_STAGE/Contents/Resources/scripts/safe_clean.sh"
chmod +x "$APP_STAGE/Contents/Resources/scripts/safe_clean.sh"
